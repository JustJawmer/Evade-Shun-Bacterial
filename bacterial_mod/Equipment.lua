...
-- ============================================================================
-- SMALL FLESH BLOB SPAWN EGG
-- ============================================================================
minetest.register_craftitem("bacterial_mod:spawn_small_flesh_blob", {
    description = "Small Flesh Blob Spawn Egg",
    inventory_image = "Small_Flesh_Blob_Spawn.png",
    on_use = function(itemstack, user, pointed_thing)
        if pointed_thing and pointed_thing.type == "node" then
            local pos = pointed_thing.above
            local ent = minetest.add_entity(pos, "bacterial_mod:small_flesh_blob")
            if ent then
                minetest.log("action", string.format("[bacterial_mod] spawn_small_flesh_blob: created bacterial_mod:small_flesh_blob at %s", minetest.pos_to_string(pos)))
                local le = ent:get_luaentity()
                if le and le.name then
                    minetest.log("action", string.format("[bacterial_mod] spawn_small_flesh_blob: luaentity name = %s", tostring(le.name)))
                else
                    minetest.log("action", "[bacterial_mod] spawn_small_flesh_blob: luaentity missing or has no name")
                end
            end
            itemstack:take_item()
            return itemstack
...
minetest.register_craftitem("bacterial_mod:spawn_infected_sheep", {
    ...
    inventory_image = "Infected_Sheep_Spawn.png",
    on_use = function(itemstack, user, pointed_thing)
        if pointed_thing and pointed_thing.type == "node" then
            local pos = pointed_thing.above
...
minetest.register_craftitem("bacterial_mod:spawn_infected_pig", {
    ...
    inventory_image = "Infected_Pig_Spawn.png",
    on_use = function(itemstack, user, pointed_thing)
        if pointed_thing and pointed_thing.type == "node" then
            local pos = pointed_thing.above
...
minetest.register_craftitem("bacterial_mod:spawn_infected_cow", {
    ...
    inventory_image = "Infected_Cow_Spawn.png",
    on_use = function(itemstack, user, pointed_thing)
        if pointed_thing and pointed_thing.type == "node" then
            local pos = pointed_thing.above
...
minetest.register_craftitem("bacterial_mod:spawn_infected_human", {
    ...
    inventory_image = "Infected_Human_Spawn.png",
    on_use = function(itemstack, user, pointed_thing)
        if pointed_thing and pointed_thing.type == "node" then
            local pos = pointed_thing.above
...
minetest.register_craftitem("bacterial_mod:spawn_nexus_stage_3", {
    ...
    inventory_image = "Nexus_Stage_3_Spawn.png",
    on_use = function(itemstack, user, pointed_thing)
        if pointed_thing and pointed_thing.type == "node" then
            local pos = pointed_thing.above
