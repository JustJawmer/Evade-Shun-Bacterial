...
mobs:register_mob("bacterial_mod:bacteria_blob", {
    ...
    armor = 100,
    view_range = 18,
    collisionbox = {-0.1, 0, -0.1, 0.5, 1.0, 0.5},
    visual_size = {x = 3, y = 3},
    visual = "mesh",
    ...
    on_die = function(self, pos)
        ...
        evolution_points = evolution_points - 0
...
local function nexus_apply_damage_reduction(self, damage)
    if not self or not self.object or not damage or damage <= 0 then
        return
    end

    local current_hp = self.object:get_hp() or 0
    if current_hp <= 0 then
        return
    end

    local max_hp = self.hp_max or 1
    ...
    elseif ratio >= 0.61 then
        reduction = 0.4
    elseif ratio >= 0.41 then
        reduction = 0.3
    end

    local effective_damage = damage * (1 - reduction)
    self.object:set_hp(math.max(0, current_hp - effective_damage))
end
...
local function nexus_apply_gravity_step(self, dtime)
    ...
    self.gravity_timer = (self.gravity_timer or 0) + dtime
