...
minetest.register_node("bacterial_mod:Infected_Soil", {
    ...
    drop = "",
    sounds = {
        footstep = {name = "flesh_hit", gain = 0.4},
        dig = {name = "flesh_hit2",  gain = 0.8},
        dug = {name = "block_break",  gain = 1.0},
}
})
...
minetest.register_on_dignode(function(pos, oldnode, digger)
    ...
    if oldnode.name ~= "bacterial_mod:mold" then
        return
    end
...
local function mold_decay(pos)
    local node = minetest.get_node_or_nil(pos)
    if node and node.name == "bacterial_mod:mold" then
        minetest.set_node(pos, {name = "air"})
        local evolution_points = tonumber(ep_storage:get_string("evolution_points")) or 0
        ep_storage:set_string("evolution_points", tostring(evolution_points + 5))
        check_phase_unlock()
    end
end

local function refresh_mold_area(player)
    if not player then
        return
    end

    local pos = vector.round(player:get_pos())
    local minp = vector.add(pos, {x = -150, y = -150, z = -150})
    local maxp = vector.add(pos, {x = 150, y = 150, z = 150})
    local mold_positions = minetest.find_nodes_in_area(minp, maxp, {"bacterial_mod:mold"}) or {}

    for _, p in ipairs(mold_positions) do
        minetest.set_node(p, {name = "bacterial_mod:mold"})
    end
end

minetest.register_on_joinplayer(function(player)
    minetest.after(5, function()
        refresh_mold_area(player)
    end)
end)
